...
if __name__ == '__main__':
    ...
    #with open(f"experiments/planning/{track_name}/replay_L2.csv", "a+") as f:
    ...
    for r in range(num_races):
        ...
        while not race_done:
            reference_latent, _, _ = encoder.get_latent(
                torch.tensor(obs / 255., dtype=torch.float32).unsqueeze(0).permute(0, 3, 1, 2).to(device))
            if len(planned_actions) == 0:
                start = time.time()
                planned_actions = replay_world_model.plan(reference_latent.cpu(), horizon=planning_horizon, k=k, deterministic=True)
