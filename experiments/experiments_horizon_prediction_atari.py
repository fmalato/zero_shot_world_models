...
if __name__ == '__main__':
    ...
    games_list = ["seaquest", "spaceinvaders"]
    for track_name in games_list:
        os.makedirs(f"experiments/prediction_horizon/atari/{track_name}/samples/one_step/", exist_ok=True)
        os.makedirs(f"experiments/prediction_horizon/atari/{track_name}/samples/long_term/", exist_ok=True)
        ...
        vae.load_state_dict(torch.load(f"vae_models/atari_{track_name}_expert-v0/encoder_{latent_size}_250.pth"))
        vae.to(device)
        vae.eval()
        ...
        ssm_short.load_state_dict(torch.load(f"ssm_models/atari/{track_name}/expert-v0/ssm_250_{num_collected}.pth"))
        ssm_short.to(device)
        ...
        ssm_long.load_state_dict(torch.load(f"ssm_models/atari/{track_name}/expert-v0/ssm_250_{num_collected}.pth"))
        ssm_long.to(device)
        freeze_parameters(ssm_long)

        train_dataset = minari.load_dataset(f"atari/{track_name}/expert-v0")
        for i, ep in enumerate(train_dataset.iterate_episodes()):
            if i < num_collected:
                obs = []
                for o in ep.observations:
                    obs.append(cv2.resize(o, img_size))
                observations = torch.tensor(np.stack(obs)).float().permute(0, 3, 1, 2).to(device)
                latents, means, logvars = vae.get_latent(observations / 255.)
                actions = torch.cat([torch.tensor(ep.actions).long(), torch.zeros(size=(1,)).long()], dim=0)
                ...
                rollout_world_model.add_trajectory(
                    ...
                    next_observations=latents_next,
                    means=means,
                    logvars=logvars
                )
                ...
                observations = np.stack([cv2.resize(x, img_size) for x in ep.observations])
                test_traj = {
                    "observations": observations,
                    "actions": ep.actions,
                    "next_observations": np.concatenate([observations[1:], np.zeros(shape=(1, 64, 64, 3))], axis=0)
                }
        ...
        for s in range(num_samples):
            ...
            for ts, tr in enumerate(similar_indices_rollout.squeeze().numpy()):
                ...
                if condition_on_action and len(imagined_obs) <= num_collected / 2:
                    imagined_obs = []
                    for ts, tr in enumerate(similar_indices_rollout.squeeze().numpy()):
                        imagined_next = rollout_world_model.get_transition(torch.tensor([ts]), torch.tensor([tr + i]))
                        imagined_obs.append(imagined_next.observation)
            ...
            # Save samples
            np.savez_compressed(
                f"experiments/prediction_horizon/atari/{track_name}/samples/one_step/decoded_one_step_sample_{s}.npz",
            ...
            np.savez_compressed(
                f"experiments/prediction_horizon/atari/{track_name}/samples/long_term/decoded_long_term_sample_{s}.npz",
                real=np.array([cv2.resize(x, (128, 128)) for x in real_seq]),
        ...
        ax[0, 1].set_yticks([0, 0.01, 0.02, 0.03])
        ax[1, 1].set_yticks([0, 0.01, 0.02, 0.03])
        ax[0, 2].set_yticks([0.7, 0.8, 0.9, 1.0])
        ...
        ax[0, 1].set_yticklabels(["0", ".01", ".02", ".03"])
        ax[1, 1].set_yticklabels(["0", ".01", ".02", ".03"])
        ax[0, 2].set_yticklabels([0.7, 0.8, 0.9, 1.0])
        ax[1, 2].set_yticklabels([0.7, 0.8, 0.9, 1.0])
        ...
        fig.savefig(f"experiments/prediction_horizon/atari/{track_name}/numerical_comparison.png")
        fig.clear()

    # Benchmark results
    fig, ax = plt.subplots(nrows=2, ncols=3, figsize=(18, 7))
    for k in overall_results_kl.keys():
        overall_results_kl[k]["mean"] = overall_results_kl[k]["mean"] / float(len(games_list))
        overall_results_kl[k]["std"] = overall_results_kl[k]["std"] / float(len(games_list))
        overall_results_l1[k]["mean"] = overall_results_l1[k]["mean"] / float(len(games_list))
        overall_results_l1[k]["std"] = overall_results_l1[k]["std"] / float(len(games_list))
        overall_results_ssim[k]["mean"] = overall_results_ssim[k]["mean"] / float(len(games_list))
        overall_results_ssim[k]["std"] = overall_results_ssim[k]["std"] / float(len(games_list))
    ...
    fig.legend([ssm_one, rollout_one, replay_l2_one, replay_kl_one], ["Baseline", "Rollout", "Replay-L2", "Replay-KL"], loc="upper right", fontsize=16)
    fig.savefig(f"experiments/prediction_horizon/atari/benchmark_numerical_comparison.png")
    fig.clear()
