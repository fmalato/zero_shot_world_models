...
if __name__ == '__main__':
    ...
    track_list = ["lighthouse", "fortmagma", "snes_rainbowroad", "snowmountain", "volcano_island"]
    for track_name in track_list:
        traj_path = f"{data_path}/{track_name}/train"
        test_traj_path = f"{data_path}/{track_name}/test"
        os.makedirs(f"experiments/prediction_horizon/super_tux_kart/{track_name}/samples/one_step/", exist_ok=True)
        os.makedirs(f"experiments/prediction_horizon/super_tux_kart/{track_name}/samples/long_term/", exist_ok=True)
        ...
        ssm_short.load_state_dict(torch.load(f"ssm_models/ssm_{track_name}_250_{num_collected}.pth"))
        ssm_short.to(device)
        ...
        ssm_long.load_state_dict(torch.load(f"ssm_models/ssm_{track_name}_250_{num_collected}.pth"))
        ssm_long.to(device)
        ...
        for s in range(num_samples):
            ...
            for i in range(1, horizon):
                ...
                for tr in similar_indices_replay.squeeze().numpy():
                    ...
                    if condition_on_action and len(imagined_obs) <= num_collected / 2:
                        imagined_obs = []
                        for ts, tr in enumerate(similar_indices_rollout.squeeze().numpy()):
                            imagined_next, _ = replay_world_model.sample_similar(latent.cpu(), action=None, k=10)
                            imagined_obs.append(imagined_next.observation)
            ...
            if save_samples:
                np.savez_compressed(
                    f"experiments/prediction_horizon/super_tux_kart/{track_name}/samples/one_step/decoded_one_step_sample_{s}.npz",
                ...
                np.savez_compressed(
                    f"experiments/prediction_horizon/super_tux_kart/{track_name}/samples/long_term/decoded_long_term_sample_{s}.npz",
                    real=np.array([cv2.resize(x, (128, 128)) for x in real_seq]),
        ...
        ax[0, 2].set_yticks([0.2, 0.4, 0.6])
        ax[1, 2].set_yticks([0.2, 0.4, 0.6])
        ...
        fig.savefig(f"experiments/prediction_horizon/super_tux_kart/{track_name}/numerical_comparison.png")
        fig.clear()
    ...
    ax[0, 1].set_yticklabels([0, 0.1, 0.2])
    ax[1, 1].set_yticklabels([0, 0.1, 0.2])
    ax[0, 2].set_yticklabels([0.2, 0.4, 0.6])
    ax[1, 2].set_yticklabels([0.2, 0.4, 0.6])

    fig.legend([ssm_one, rollout_one, replay_l2_one, replay_kl_one], ["Baseline", "Rollout", "Replay-L2", "Replay-KL"], loc="upper right", fontsize=16)
    fig.savefig(f"experiments/prediction_horizon/super_tux_kart/benchmark_numerical_comparison.png")
    fig.clear()
