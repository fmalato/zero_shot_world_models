...
from vae import VAE
from ssm import NextStepPredictionSSM
...
if __name__ == '__main__':
    ...
    track_list = ["fortmagma", "snes_rainbowroad"]
    num_trajectories = [5, 6, 7, 8, 9, 10, 15, 20, 25, 30]

    total_heatmap_kl_one_step = np.zeros(shape=(4, len(num_trajectories)))
    total_heatmap_kl_long_term = np.zeros(shape=(4, len(num_trajectories)))
    total_heatmap_l1_one_step = np.zeros(shape=(4, len(num_trajectories)))
    total_heatmap_l1_long_term = np.zeros(shape=(4, len(num_trajectories)))
    total_heatmap_ssim_one_step = np.zeros(shape=(4, len(num_trajectories)))
    total_heatmap_ssim_long_term = np.zeros(shape=(4, len(num_trajectories)))
    ...
    for track_name in track_list:
        ...
        indices = list(np.random.randint(ssm_context_length, test_traj["observations"].shape[0] - horizon - 1, size=num_samples))
        ...
        for traj_idx, num_collected in enumerate(num_trajectories):
            ...
            ssm_short = NextStepPredictionSSM(
                encoded_latent_size=latent_size,
                action_size=num_actions,
                hidden_state_size=hidden_state_size,
                context_size=ssm_context_length
            )
            ssm_short.load_state_dict(torch.load(f"ssm_models/ssm_ablation/{track_name}/ssm_{track_name}_250_{num_collected}.pth"))
            ssm_short.to(device)
            freeze_parameters(ssm_short)

            ssm_long = NextStepPredictionSSM(
                encoded_latent_size=latent_size,
                action_size=num_actions,
                hidden_state_size=hidden_state_size,
                context_size=ssm_context_length
            )
            ssm_long.load_state_dict(torch.load(f"ssm_models/ssm_ablation/{track_name}/ssm_{track_name}_250_{num_collected}.pth"))
            ssm_long.to(device)
            ...
            for s, tr_idx in enumerate(indices):
                ...
                ssm_short_pred, stats_short = ssm_short.predict(h_t, o_t.unsqueeze(0), torch.tensor(test_traj["actions"][tr_idx]).view(1, 1).to(device), device)
                ...
                for i in range(1, horizon):
                    ...
                    o_t = torch.cat(ssm_obs_buffer, dim=0)
                    o_t, _, _ = vae.get_latent((o_t / 255.).to(device))
                    ssm_short_pred, stats_short = ssm_short.predict(h_t, o_t.unsqueeze(0), torch.tensor(test_traj["actions"][tr_idx + i]).view(1, 1).to(device), device)
                    ssm_short_term_seq.append(ssm_short_pred[0])
                    ssm_short_stats_seq.append(stats_short)
                    # Replay L2 1-step
                ...
                o_t = torch.cat(ssm_obs_buffer, dim=0)
                o_t, _, _ = vae.get_latent((o_t / 255.).to(device))
                ssm_long_term_seq, stats_long = ssm_long.predict(h_t, o_t.unsqueeze(0), torch.tensor(test_traj["actions"][tr_idx :tr_idx + horizon]).view(1, horizon).to(device), device, horizon=horizon)
                context_obs, context_stats = ssm_short.get_context_prior(h_t, o_t.unsqueeze(0), torch.tensor(test_traj["actions"][tr_idx]).view(1, 1).to(device), device)
                ...
                # Plot samples
                # 1-step
                os.makedirs(f"experiments/trajectories_ablation/super_tux_kart/{track_name}/{num_collected}/samples/one_step/", exist_ok=True)
                np.savez_compressed(
                    f"experiments/trajectories_ablation/super_tux_kart/{track_name}/{num_collected}/samples/one_step/decoded_one_step_sample_{s}.npz",
                ...
                os.makedirs(f"experiments/trajectories_ablation/super_tux_kart/{track_name}/{num_collected}/samples/long_term/", exist_ok=True)
                np.savez_compressed(
                    f"experiments/trajectories_ablation/super_tux_kart/{track_name}/{num_collected}/samples/long_term/decoded_long_term_sample_{s}.npz",
                    real=np.array([cv2.resize(x, (128, 128)) for x in real_seq]),
                ...
                """fig, ax = plt.subplots(nrows=5, ncols=horizon, figsize=(15.5, 4))
                row_labels = ["Actual", "SSM", "Rollout", "Replay-L2", "Replay-KL"]
                y_positions = [0.8825, 0.68875, 0.5, 0.3125, 0.12]  # Adjust based on visual alignment
    
                for label, y in zip(row_labels, y_positions):
                    fig.text(0.0065, y, label, va='center', ha='center', rotation='vertical', fontsize=7)
    
                for i in range(horizon):
                    ax[0, i].imshow(real_seq[i])
                    ax[1, i].imshow(decoded_ssm_short[i].cpu().numpy().transpose(1, 2, 0))
                    ax[2, i].imshow(decoded_rollout_one[i].cpu().numpy().transpose(1, 2, 0))
                    ax[3, i].imshow(decoded_replay_l2_one[i].cpu().numpy().transpose(1, 2, 0))
                    ax[4, i].imshow(decoded_replay_kl_one[i].cpu().numpy().transpose(1, 2, 0))
                    ax[0, i].axis("off")
                    ax[1, i].axis("off")
                    ax[2, i].axis("off")
                    ax[3, i].axis("off")
                ...
                for y in [0.7875, 0.6]:  # Adjust as needed to sit between rows
                    line = Line2D([0.005, 0.995], [y, y], transform=fig.transFigure, color="black", linewidth=0.5)
                    fig.add_artist(line)
    
                plt.tight_layout()
                fig.savefig(f"experiments/trajectories_ablation/super_tux_kart/{num_collected}_trajectories/{track_name}/samples/one_step/sample_{s}.png")
                fig.clear()
                # Long-term
                os.makedirs(f"experiments/trajectories_ablation/super_tux_kart/{num_collected}_trajectories/{track_name}/samples/long_term/", exist_ok=True)
                fig, ax = plt.subplots(nrows=5, ncols=horizon, figsize=(15.5, 4))
                row_labels = ["Actual", "SSM", "Rollout", "Replay-L2", "Replay-KL"]
                y_positions = [0.8825, 0.68875, 0.5, 0.3125, 0.12]
                ...
                for i in range(horizon):
                    ax[0, i].imshow(real_seq[i])
                    ax[1, i].imshow(decoded_ssm_long[i].cpu().numpy().transpose(1, 2, 0))
                    ax[2, i].imshow(decoded_rollout_long[i].cpu().numpy().transpose(1, 2, 0))
                    ax[3, i].imshow(decoded_replay_l2_long[i].cpu().numpy().transpose(1, 2, 0))
                    ax[4, i].imshow(decoded_replay_kl_long[i].cpu().numpy().transpose(1, 2, 0))
                    ax[0, i].axis("off")
                    ax[1, i].axis("off")
                    ax[2, i].axis("off")
                    ax[3, i].axis("off")
                ...
                plt.tight_layout()
                fig.savefig(f"experiments/trajectories_ablation/super_tux_kart/{num_collected}_trajectories/{track_name}/samples/long_term/sample_{s}.png")
                fig.clear()"""
            ...
            for measure in ["kl", "l1", "ssim"]:
                #fig, ax = plt.subplots(nrows=2, ncols=1, figsize=(17, 7))
                # Rollout one-step
        ...
        for m_one, m_long, name, lab, cmap in zip(maps_one_step, maps_long_term, names, barlabel, colormaps):
            ...
            fig.savefig(f"experiments/trajectories_ablation/super_tux_kart/{track_name}/heatmap_{name}.png", bbox_inches='tight')

        total_heatmap_kl_one_step += heatmap_kl_one_step
        total_heatmap_kl_long_term += heatmap_kl_long_term
        total_heatmap_l1_one_step += heatmap_l1_one_step
        total_heatmap_l1_long_term += heatmap_l1_long_term
        total_heatmap_ssim_one_step += heatmap_ssim_one_step
        total_heatmap_ssim_long_term += heatmap_ssim_long_term

    # Plot total heatmaps
    total_heatmap_kl_one_step = total_heatmap_kl_one_step / len(track_list)
    total_heatmap_kl_long_term = total_heatmap_kl_long_term / len(track_list)
    total_heatmap_l1_one_step = total_heatmap_l1_one_step / len(track_list)
    total_heatmap_l1_long_term = total_heatmap_l1_long_term / len(track_list)
    total_heatmap_ssim_one_step = total_heatmap_ssim_one_step / len(track_list)
    total_heatmap_ssim_long_term = total_heatmap_ssim_long_term / len(track_list)

    # Plot heatmaps - KL
    labels = ["SSM", "Rollout", "Replay-L2", "Replay-KL"]
    maps_one_step = [total_heatmap_kl_one_step, total_heatmap_l1_one_step, total_heatmap_ssim_one_step]
    maps_long_term = [total_heatmap_kl_long_term, total_heatmap_l1_long_term, total_heatmap_ssim_long_term]
    names = ["kl", "l1", "ssim"]
    barlabel = ["KL divergence", "L1 distance", "SSIM"]
    colormaps = ["YlGnBu", "YlGn", "YlOrRd"]
    for m_one, m_long, name, lab, cmap in zip(maps_one_step, maps_long_term, names, barlabel, colormaps):
        ...
        fig.savefig(f"experiments/trajectories_ablation/super_tux_kart/heatmap_{name}.png", bbox_inches='tight')
