...
class RSSMPolicy:
    ...
    def _poll(self, obs, action=None):
    ...
    def poll(self, observation, action=None, explore=False):
        with torch.no_grad():
            self._poll(observation, action)
            if explore:
                self.a += torch.randn_like(self.a)*0.3
            return self.a


class RSSMTuxKartPolicy(RSSMPolicy):
    def __init__(self,
            model,
            planning_horizon,
            num_candidates,
    ...
    def _poll(self, obs, action=None):
        self.probs = (torch.ones(size=(self.H, self.d)) / self.d).to(self.device)
        # observation could be of shape [CHW] but only 1 timestep
        assert len(obs.shape) == 3, 'obs should be [CHW]'
        self.h, self.s = self.rssm.get_init_state(
            self.rssm.encoder(obs[None]),
            self.h, self.s, F.one_hot(torch.argmax(self.a), num_classes=self.d).unsqueeze(0)
        )
        for _ in range(self.T):
            rwds = torch.zeros(self.N).to(self.device)
            actions = Categorical(self.probs).sample((self.N,)).to(self.device)
            h_t = self.h.clone().expand(self.N, -1)
            s_t = self.s.clone().expand(self.N, -1)
            for a_t in torch.unbind(actions, dim=1):
                h_t = self.rssm.deterministic_state_fwd(h_t, s_t, F.one_hot(a_t, num_classes=self.d))
                s_t = self.rssm.state_prior(h_t, sample=True)
                rwds += self.rssm.pred_reward(h_t, s_t)
            _, k = torch.topk(rwds, self.K, dim=0, largest=True, sorted=False)
            self.probs = torch.stack([torch.bincount(actions[k, i], minlength=self.d) / k.size(0) for i in range(self.H)])
        if action is None:
            self.a = self.probs[0]
        else:
            self.a = action

    def poll(self, obs, action=None, explore=False):
        self._poll(obs, action)
        if explore:
            return Categorical(self.a).sample()
        else:
            return torch.argmax(self.a)