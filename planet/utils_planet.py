...
def preprocess_img(image, depth, inplace=True):
    """
...
class TuxKartTorchImageEnvWrapper:
    """
    Torch Env Wrapper that wraps a gym env and makes interactions using Tensors.
    Also returns observations in image form.
    """
    def __init__(self, env, bit_depth, observation_shape=None, act_rep=1):
        self.env = env
        self.bit_depth = bit_depth
        self.action_repeats = act_rep
    ...
    def step(self, u):
        u, rwds = u.cpu().detach().numpy(), 0
        tux_act = self.env.unwrapped.ACTIONS[int(u)]
        u = pystk.Action(acceleration=tux_act[0], brake=tux_act[1], steer=tux_act[2])
        for _ in range(self.action_repeats):
            o, r, terminated, truncated, i = self.env.step(u)
            rwds += r
        x = to_tensor_obs(o)
        preprocess_img(x, self.bit_depth)
        return x, rwds, terminated, truncated, i
